...
class RegisteredApiBufferManagerWorker extends BufferManagerWorker_1.default {
  constructor(config, ctUrl) {
    super(config, ctUrl);
    this.semaphore = new Semaphore_1.default(
      this.getRegisteredApiCountToCapture()
    );
  }
  init() {
    return true;
  ...
  offer(key, apiSample) {
    this.bufferMap.computeIfAbsent(
      key,
      new SimpleBuffer_1.default(this.getRegisteredApiBufferSize(key))
    );
  ...
  canOffer(key) {
    var _a;
    if (
      !((_a = this.getOperatingConfig()) === null || _a === void 0
        ? void 0
        : _a.getCaptureApiSample())
    ) {
      return false;
    }
  ...
  getRegisteredApiBufferSize(apiBufferKey) {
    ...
    for (let apiConfig of agentConfig.getRegisteredApiConfigs()) {
      if (
        apiConfig.getMethod() === apiBufferKey.getMethod() &&
  ...
  syncForKey(key) {
    try {
      ...
      while (iterations-- > 0) {
        ...
        (0, RequestExecutor_1.postRequest)(this.ctUrl + this.getUri(), [
          apiSample,
        ])
          .then(() => {
            console.log("synced register");
          })
          .catch((error) => {
            SDKLogger_1.default.error(
              "While sending registered api sample: " + error
            );
          });
      }
    } catch (e) {
